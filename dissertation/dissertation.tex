...
Stretches of adenines between five and nine nucleotides were depleted in libraries for DNA bound to nucleosomes, as indicated by normalised frequencies below 1.0 (figure~\ref{fig:astretch}).  This agrees with previous studies that reveal that poly(dA) tracts disfavour nucleosome positioning because they are stiff \citep{struhl_determinants_2013}.  Although the trends in the data suggested that methylated DNA exhibited a greater depletion of A-stretches than plain DNA, this could not be concluded as the signals from plain DNA are weak.  The data also present a trend of increased enrichment of A-stretches as the A-stretches are longer, especially in the plain DNA libraries, but this can also be attributed to the weak signals in the plain DNA libraries.  Previous experiments with plain DNA libraries enriched for signal suggested that A-stretches are depleted as their length increases, concordant with previous theories regarding poly(dA) tracts.

In sum, the analyses in this project confirmed previous theories on nucleosome sequence preferences.  Namely: there is a 10-bp periodicity in sequence preferences, TA sequences occur out of phase with GC sequences, and poly(dA) tracts disfavour nucleosome positioning.  Results therefore confirmed that nucleosome EMSA SELEX with the sequencing methods employed revealed the same conclusions as previous MNase-based assays.  % something about physiological relevance to 'broaden' the end of this section??
...
Additionally, cytosine methylation was observed to be associated with enrichment of certain dinucleotides containing cytosines (figure~\ref{fig:c2nt}), but the effect size is small.  These small deviations from 1:1 ratios can be reasoned with the low affinity of histone octamers to DNA, at least relative to transcription factors \citep{struhl_determinants_2013}. % probably find better citation to support this assertion, admittedly the sentence i was thinking of is not that strong
